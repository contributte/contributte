# Contributte README generator v2
...
8. For a frontend skeleton, put the canonical interactive dev-server route before passive watch/build alternatives when repository evidence supports it. Separate the browser URL, proxy behavior, and environment overrides when readers need them independently.
9. Include an evidence-backed maintenance or ownership route and contribution route for a maintained repository. Do not infer or invent a maintainer from commit history.
10. Do not edit files, execute destructive commands, commit, push, create or modify a pull request, or communicate externally.
11. Return only the complete candidate between these exact markers:
